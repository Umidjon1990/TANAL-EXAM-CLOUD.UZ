...
export const createUserSchema = z.object({
  username: z
    ...
    .min(3, "Foydalanuvchi nomi kamida 3 belgidan iborat bo'lishi kerak")
    .max(50)
    .regex(/^[a-zA-Z0-9_.-]+$/, "Faqat harf, raqam, _ . - belgilaridan foydalaning"),
...
export const createTestCenterSchema = z.object({
  ...
  region: z.enum(REGIONS, { errorMap: () => ({ message: "Viloyatni tanlang" }) }),
...
export const createExamSchema = z.object({
  ...
  capacity: z.coerce.number().int().positive("Joylar soni musbat bo'lishi kerak").optional(),
  price: z.coerce.number().int().nonnegative("Narx manfiy bo'lmasligi kerak").optional(),
...
export const rejectExamSchema = z.object({
  ...
  reason: z.string().min(5, "Rad etish sababini kiriting (kamida 5 belgi)").max(500),
});
